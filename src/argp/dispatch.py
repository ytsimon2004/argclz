...
>>> from argp import AbstractParser
...
DISPATCH_INFO = '__DISPATCH_INFO__'
...
def dispatch(command: str,
    ...
    def _dispatch(f):
        if hasattr(f, DISPATCH_INFO):
            raise RuntimeError()

        setattr(f, DISPATCH_INFO, DispatchCommand(command, tuple(alias), group, f))
        return f
...
def list_commands(host: Union[T, Type[T]], group: Optional[str] = missing) -> list[DispatchCommand]:
    ...
    for attr in dir(host_type):
        ...
        if (info := getattr(attr_value, DISPATCH_INFO, None)) is not None:
